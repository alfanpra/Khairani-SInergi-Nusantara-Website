  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50">
      <div className="relative w-full">
        <div ref={navBgRef} className="absolute inset-0" aria-hidden="true" />
        <nav
                ...
                <li key={path}>
                  <Link
                    ...
                    className={`relative font-heading text-[13px] font-medium uppercase tracking-[0.12em] transition-colors hover:text-amber-400 ${
                      isCurrent ? 'text-amber-500' : 'text-white-soft/60'
                    }`}
                  >
                    {label}
                    {isCurrent && (
                      <span className="absolute -bottom-1.5 left-0 h-px w-full bg-amber-500" aria-hidden="true" />
                    )}
                  </Link>
                </li>
              )
      ...
      >
        <div className="flex h-full flex-col px-6 pb-10 pt-24">
                ...
                >
                  <Link
                    ...
                    className={`block py-4 font-heading text-2xl font-semibold tracking-tight transition-colors ${
                      isCurrent ? 'text-amber-500' : 'text-white-soft/80'
                    }`}
          ...
          <div className="mt-auto space-y-4">
            ...
            <a href={`tel:${COMPANY.phone.replace(/\s/g, '')}`} className="btn-ghost w-full">
              {COMPANY.phone}
            </a>
          </div>
        </div>
